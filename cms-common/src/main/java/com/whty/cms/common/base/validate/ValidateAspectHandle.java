package com.whty.cms.common.base.validate;
...
public class ValidateAspectHandle {
    ...
    @Around("@annotation(com.whty.cms.common.base.validate.ValidateGroup)")  
    ...
    public boolean validateFiled( ValidateFiled[] valiedatefiles , Object[] args) throws SecurityException, IllegalArgumentException, NoSuchMethodException, IllegalAccessException, InvocationTargetException{  
        for (ValidateFiled validateFiled : valiedatefiles) {  
            ...
            if(validateFiled.notNull()){        //判断参数是否为空  
                if(arg == null )  
                    return false;  
                if (arg instanceof String && CheckEmpty.isEmpty(arg.toString()))
                	return false;
            ...
            if (validateFiled.solrParam()){
            	if (arg!=null){
            		String str = (String)arg;
            		if (CheckEmpty.isNotEmpty(validateFiled.excludeParam())){
            			String[] excludeParams = validateFiled.excludeParam().split(",");
